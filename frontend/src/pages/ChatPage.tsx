...
import { useThreads, useMessages, useChatStore } from "../stores";
...
const ChatPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    console.log(`Loading threads: ${hasPersistedThreads ? 'silent refresh' : 'initial load'} (${threads.length} persisted threads)`);
    ...
    if (isNewChat) {
      ...
      loadThreadMessages(threadId).catch(error => {
        ...
        if (isAuthError(error)) {
          console.log('Session expired while loading thread, redirecting to new chat');
          navigate('/chat', { replace: true });
        }
        // If thread doesn't exist anymore, redirect to new chat
        else if (error?.response?.status === 404) {
          console.log('Thread not found, redirecting to new chat');
          navigate('/chat', { replace: true });
        }
      });
    }
  ...
  const handleSendMessage = async (message: string) => {
    try {
      // Use experiment_type = "control" for this page
      const response = await sendMessageToAPI(message, true, "control");

      // If this was a new chat and we got a thread_id, update the URL without navigation
      if (isNewChat && response?.thread_id) {
        const url = `/chat/${response.thread_id}`;
